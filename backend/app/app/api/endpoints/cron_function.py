...
async def invite_mails(db:Session=Depends(deps.get_db)):
    ...
    for notify in get_notification:
        if notify.type == 1 : # SMS
            mobile_no=str(notify.mobile_no_email_id).split(',') if notify.mobile_no_email_id else []
            for mobile in mobile_no:
                if mobile != '':
                    send_sms=sendSMS(notify.mobile_no_email_id,notify.message)
            
        elif notify.type == 2: # Mail
            mails=str(notify.mobile_no_email_id).split(',') if notify.mobile_no_email_id else []
            for mail in mails:
                if mails != "":
                    send_mail=await send_email(db,mail,notify.subject,notify.message)
