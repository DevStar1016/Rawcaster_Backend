...
async def signup(db:Session=Depends(deps.get_db),signup_type:str=Form(1,description="1-Email,2-Phone Number"),first_name:str=Form(None,max_length=100),
    ...
    if auth_code == None or auth_code.strip() == "":    
        ...
        if checkAuthCode(auth_code,auth_text) == False:
            ...
            check_user=db.query(User).filter(or_(
            ...
            if check_user:
                ...
                if add_user:
                    ...
                    if ref_id:
                        ...
                        if len(referrer_ref_id) == 2:
                            ...
                            if referred_user:
                                ...
                                ref_friend=MyFriends(sender_id=referred_user.id,receiver_id=add_user.id,request_date=datetime.datetime.utcnow(),request_status=1,status_date=None,status=1)
                                db.add(ref_friend)
                                db.commit()
                                ...
                                if get_friend_group:
                                    add_follow_user=FollowUser(follower_userid=add_user.id,following_userid=referred_user.id,created_date=datetime.datetime.utcnow())
                                    ...
                                    if not friend_group_member:
                                        add_friend_group_member=FriendGroupMembers(group_id=get_friend_group.id,user_id=add_user.id,added_date=datetime.datetime.utcnow(),added_by=referred_user.id,is_admin=0,disable_notification=1,status=1)
                    ...
                    if rawcaster_support_id > 0 and referred_id != rawcaster_support_id :
                        add_my_friends=MyFriends(sender_id=rawcaster_support_id,receiver_id=add_user.id,request_date=datetime.datetime.utcnow(),request_status=1,status_date=None,status=1)
...
async def signupverify(db:Session=Depends(deps.get_db),auth_code:str=Form(None,description="SALT + otp_ref_id"),otp_ref_id:str=Form(None,description="From service no. 1"),otp:str=Form(None),otp_flag:str=Form(None),alt_token_id:str=Form(None)):
    if auth_code == None or auth_code.strip() == "":
        return {"status":0,"msg":"Auth Code is missing"}
    ...
    if checkAuthCode(auth_code,auth_text) == False:
        ...
        if not otp_ref_id:
            if not token and token.strip() == "":
                ...
                add_model=OtpLog(user_id=login_user_id,otp=otp,otp_type=1,created_at=otp_time,status=1)
                db.add(add_model)
                db.commit()
                if add_model:
        ...
        if not get_otp_log:
            ...
            if otp_flag == "sms":
                ...
                if get_otp_log.otp_type == 1:
                    content += f'Click this link to validate your account {link} <br /><br />'
                else:
                    content += '<br />'
        ...
        if access_token == False:
            ...
            if get_token_details:
                if get_token_details.device_type == 2:
                    ...
                    # Update Friend Chat
                    update_friend_sender_chat=db.query(FriendsChat).filter(FriendsChat.sender_id == user_id).update({"sender_delete":1,"sender_deleted_datetime":datetime.datetime.utcnow()})
                    update_friend_receiver_chat=db.query(FriendsChat).filter(FriendsChat.receiver_id == user_id).update({"receiver_delete":1,"receiver_deleted_datetime":datetime.datetime.utcnow()})
                    db.commit()
        ...
        if checkAuthCode(auth_code,auth_text) == False:
            ...
            if not get_user:
                ...
                if username.isnumeric():
                    to=f"{get_user.country_code}{get_user.mobile_no}" if get_user.mobile_no else None
                    sms=f"{otp} is your OTP for Rawcaster. PLEASE DO NOT SHARE THE OTP WITH ANYONE."
                ...
                return {"status":1,"msg":msg,"otp_ref_id":otp_ref_id,"remaining_seconds":90} # remaining_seconds
        ...
        if access_token == False:
            ...
            if checkAuthCode(auth_code,auth_text) == False:
                ...
                if check_phone > 0:
                    ...
                    # get_user_profile.profile_img = defaultimage('profile_img')
                    # get_user_profile.cover_image = defaultimage('cover_img')
    ...
    if token == None or token.strip() == "":
        ...
        if checkAuthCode(auth_code.strip(),auth_text) == False:
            ...
            if access_token == False:
                ...
                get_user=db.query(User).filter(User.status == 1,User.id != login_user_id)
                ...
                if user_ids == "" or user_ids == None:
                    ...
                    for user in user_ids:
                        ...
                        if users:
                            ...
                            request_statuss=[0,1]
                            get_my_friends=db.query(MyFriends).filter(or_(MyFriends.sender_id == login_user_id,MyFriends.sender_id == user_id),or_(MyFriends.receiver_id == user_id,MyFriends.receiver_id == login_user_id),MyFriends.request_status.in_(request_statuss)).order_by(MyFriends.id.desc())
    ...
    if token == None or token.strip() == '':
        ...
        if not access_token:
            ...
            if not my_friends:
                ...
                update_my_friends=db.query(MyFriends).filter(MyFriends.id == friend_request_id).update({"status":status,"request_status":response,"status_date":datetime.datetime.utcnow()})
                db.commit()
                ...
                if update_my_friends:
                    if notification_id:
                        ...
                        update_notification=db.query(Notification).filter(Notification.notification_origin_id == my_friends.sender_id,Notification.user_id ==my_friends.receiver_id,Notification.notification_type == 11).update({"status":0,"is_read":1,"read_datetime":datetime.datetime.utcnow()})
                        db.commit()
...
async def listallfriends(db:Session=Depends(deps.get_db),token:str=Form(None),search_key:str=Form(None),group_ids:str=Form(None,description="Like ['12','13','14']"),
                         ...
                         age:str=Form(None),gender:str=Form(None),location:str=Form(None),user_id:str=Form(None),
        ...
        if access_token == False:
            ...
            if gender:
                if not gender.isnumeric():
                    ...
                    get_my_friends=get_my_friends.filter(or_(MyFriends.sender_id.in_(get_user_ids),MyFriends.receiver_id.in_(get_user_ids)))
        ...
        if (share_type == 3 or share_type == 4 or share_type == 5) and not share_with:
            ...
            if anyissue == 1:
                ...
                for i in range(looping_count):
                    add_nuggets_master=NuggetsMaster(user_id=login_user_id,content=content if content_location == 0 else None,_metadata=metadata,poll_duration=poll_duration,created_date=datetime.datetime.utcnow(),status=0)
                    db.add(add_nuggets_master)
                    db.commit()
                    ...
                    if add_nuggets_master:
                        ...
                        if nuggets_media:
                            ...
                            file_name=nuggets_media[i-1].filename
                            file_temp=nuggets_media[i-1].content_type
                            
                            # File Upload
                            file_ext = os.path.splitext(nuggets_media[i-1].filename)[1]
                            
                            uploaded_file_path=await file_upload(nuggets_media[i-1],file_ext,compress=1)
                ...
                return {"status":1,"msg":"Nuggets created successfully!","nugget_detail":sorted(nugget_details, key=lambda x: x["nugget_id"])}
        ...
        if access_token == False:
            ...
            if access_token == 'RAWCAST':  # When Customer not login
                ...
                if nugget_type == 1:  # Video Nugget
                    ...
                    get_nuggets=get_nuggets.join(NuggetsAttachment,Nuggets.nuggets_id == NuggetsAttachment.nugget_id,isouter=True).filter(or_(NuggetsAttachment.media_type == None,NuggetsAttachment.media_type == 'image',NuggetsAttachment.media_type == 'audio'))
            ...
            if get_nuggets_count < 1:
                ...
                for nuggets in get_nuggets:
                    ...
                    nuggets_list.append({
                                        ...
                                        "original_user_image":nuggets.nuggets_master.user.profile_img if nuggets.user_id  else "",
                                        "user_name":nuggets.user.display_name if nuggets.user_id  else "",
                                        "user_image":nuggets.user.profile_img if nuggets.user_id  else "",
                                        "user_status_id":nuggets.user.user_status_id if nuggets.user_id  else "",
                                        "liked":nugget_like,
                                        "viewed":0,
...
async def addnuggetcomment(db:Session=Depends(deps.get_db),token:str=Form(None),type:str=Form(None,description="1-comment,2-reply"),nugget_id:str=Form(None),
    ...
    if token == None or token.strip() == "":
        ...
        if access_token == False:
            ...
            check_nuggets=db.query(Nuggets).filter_by(id = nugget_id).first()
            if check_nuggets:
                nugget_comment=NuggetsComments(user_id=login_user_id,parent_id=comment_id,nugget_id=nugget_id,content=comment,created_date=datetime.datetime.utcnow(),modified_date=datetime.datetime.utcnow())
                ...
                if nugget_comment:
                    ...
                    result_list.update({
                                        ...
                                        "editable":True,
                                        "name":(nugget_comment.user.display_name if nugget_comment.user.display_name else "") if nugget_comment.user_id else "",
                                        "profile_image":(nugget_comment.user.profile_img if nugget_comment.user.profile_img else defaultimage('profile_img')) if nugget_comment.user_id else "",
                                        "comment":nugget_comment.content if nugget_comment.content else "",
                                        "commented_date":common_date(nugget_comment.created_date) if nugget_comment.created_date else "",
                                        "liked":False,
                                        "like_count":0
                                        })
                    if type == 1:
...
async def editnuggetcomment(db:Session=Depends(deps.get_db),token:str=Form(None),comment_id:str=Form(None),comment:str=Form(None)):
    if token == None or token.strip() == "":
        ...
        if access_token == False:
            ...
            check_nugget_comment=db.query(NuggetsComments).filter(NuggetsComments.user_id == login_user_id, NuggetsComments.id == comment_id).first()
        ...
        if token == None or token.strip() == "":
            ...
            if access_token == False:
                ...
                if IsAccountVerified(db,login_user_id) == False:
                    ...
                    check_nugget_comment=db.query(NuggetsComments).filter_by(user_id=login_user_id,id=comment_id).first()

                    if check_nugget_comment:
                        # Delete Comment Likes
                        del_comment_like=db.query(NuggetsCommentsLikes).filter_by(comment_id = check_nugget_comment.id).delete()
                        # Nugget Comments Reply
                        get_nugget_comments=db.query(NuggetsComments).filter_by(parent_id = check_nugget_comment.id).all()
                        comment_ids=[nugg_cmt.id for nugg_cmt in get_nugget_comments]
                        # Delete Comment Likes
                        del_comment_likes=db.query(NuggetsCommentsLikes).filter(NuggetsCommentsLikes.comment_id.in_(comment_ids)).delete()
                        
                        # Nugget Comments
                        delete_comment=db.query(NuggetsComments).filter(NuggetsComments.parent_id == check_nugget_comment.id).delete()
                        
                        delete_all_comment=db.query(NuggetsComments).filter(NuggetsComments.id == check_nugget_comment.id).delete()
                        db.commit()
...
async def likeandunlikenuggetcomment(db:Session=Depends(deps.get_db),token:str=Form(None),comment_id:str=Form(None),like:str=Form(None,description="1->ilike 2->unlike")):
        if token == None or token.strip() == "":
            return {"status":-1,"msg":"Sorry! your login session expired. please login again."}
        elif comment_id == None or not comment_id.isnumeric():
            ...
            if access_token == False:
                ...
                if IsAccountVerified(db,login_user_id) == False:
                    ...
                    check_nuggets_comment=db.query(NuggetsComments).filter_by(id=comment_id).first()
            ...
            if check_nuggets:
                ...
                if delete_media_id and (getmediacount != len(delete_media_id)):
                    ...
                    if anyissue == 1:
                        ...
                        update_nuggets=db.query(Nuggets).filter_by(id = check_nuggets.id).update({"share_type":share_type,"modified_date":datetime.datetime.utcnow()})
                        db.commit()
                        if update_nuggets:
                            totalmembers=[]
                            ...
                            update_nugget_master=db.query(NuggetsMaster).filter(NuggetsMaster.id == check_nuggets.nuggets_id).update({"content":content,"_metadata":metadata,"modified_date":datetime.datetime.utcnow()})
                            db.commit()
        ...
        if access_token == False:
            ...
            if (share_type == 3 or share_type == 4 or share_type == 5) and not share_with:
                        ...
                        for key,val in share_with.items():
                            ...
                            if val:
                                if key == 'groups' and (share_type == 3 or share_type == 5):
                                    ...
                                    my_friend_count = [x for x in val if x not in get_friends]
                ...
                if anyissue == 1:
                    ...
                    if check_nugget:
                        share_nugget=Nuggets(nuggets_id=check_nugget.nuggets_id,user_id=login_user_id,type=2,share_type=share_type,created_date=datetime.datetime.utcnow())
                        db.add(share_nugget)
                        db.commit()
                        totalmembers=[]
                        if share_nugget:
...
async def addevent(db:Session=Depends(deps.get_db),token:str=Form(None),event_title:str=Form(None),event_type:str=Form(None,description="1-Event,2-Talkshow,3-Live"),event_start_date:Any=Form(None),event_start_time:Any=Form(None),
                   event_message:str=Form(None),event_participants:str=Form(None),event_duration:Any=Form(None),event_layout:str=Form(None),
                   event_host_audio:str=Form(None),event_host_video:str=Form(None),event_guest_audio:str=Form(None),event_guest_video:str=Form(None),event_melody:UploadFile=File(None),
    ...
    if token == None or token.strip() == "":
        ...
        if access_token == False:
            ...
            if event_participants < 2:
                ...
                flag='event_banner' if event_type == 1 else "talkshow" if event_type == 2 else "live" if event_type == 3 else None
                
                cover_img=defaultimage(flag)
                if setting:
                    cover_img=setting.meeting_header_image if setting.meeting_header_image else defaultimage(flag)
                ...
                reference_id=f"RC{random.randint(1,499)}{datetime.datetime.utcnow().timestamp()}"
                ...
                new_event=Events(title=event_title,ref_id=reference_id,server_id=server_id if server_id != None else None,event_type_id=event_type,
                                 description=event_message,event_layout_id=event_layout,no_of_participants=event_participants,duration=event_duration,start_date_time=datetime_str,
                                 created_at=datetime.datetime.utcnow(),created_by=login_user_id,cover_img=cover_img,waiting_room=waiting_room,join_before_host=join_before_host,
                ...
                if new_event:
                    ...
                    if event_banner:
                        ...
                        uploaded_file_path=await file_upload(event_banner,file_ext,compress=None)
                    ...
                    if event_melody:
                        ...
                        uploaded_file_path=await file_upload(event_melody,file_ext,compress=None)
                        file_stat = os.stat(uploaded_file_path)
                        file_size= file_stat.st_size
                        ...
                        if file_size > 1000000 and type == 'image' and file_ext != '.gif':
                            ...
                            if result and result['status'] == 1:
                                new_melody=EventMelody(event_id=new_event.id,path=result['url'],type=media_type,created_at=datetime.datetime.utcnow(),created_by=login_user_id)
                                db.add(new_melody)
                                db.commit()
...
async def listevents(db:Session=Depends(deps.get_db),token:str=Form(None),user_id:str=Form(None),event_type:str=Form(0,description="1->My Events, 2->Invited Events, 3->Public Events"),type_filter:str=Form(None,description="1 - Event,2 - Talkshow,3 - Live"),page_number:str=Form(default=1)):
    if token == None or token.strip() == "":
        ...
        if access_token == False:
            ...
            if event_type == 4:
                event_list=event_list.filter(Events.start_date_time < datetime.datetime.utcnow())
            
            elif event_type == 5:
                event_list=event_list
            
            else:
                event_list=event_list.filter(text("DATE_ADD(events.start_date_time, INTERVAL SUBSTRING(events.duration, 1, CHAR_LENGTH(events.duration) - 3) HOUR_MINUTE) > :current_datetime")).params(current_datetime=datetime.datetime.utcnow())           
            ...
            if get_row_count < 1:
                ...
                if event_list:
                    for event in event_list:
                        ...
                        banner_image= (settings.meeting_header_image if not event.cover_img else event.cover_img) if event.type == 1 else (settings.talkshow_event_banner if not event.cover_img else event.cover_img) if event.type == 2 else (settings.live_event_banner if not event.cover_img else event.cover_img) if event.type == 3 else event.cover_img
                ...
                if edit_event:
                    ...
                    if event_melody:
                        ...
                        if file_size > 1000000 and type == 'image' and file_ext != '.gif':
                            ...
                            if result and result['status'] == 1:
                                new_melody=EventMelody(event_id=edit_event.id,path=result['url'],type=media_type,created_at=datetime.datetime.utcnow(),created_by=login_user_id)
                                db.add(new_melody)
                                db.commit()
                                db.refresh(new_melody)
                    ...
                    if event_invite_friends:
                        ...
                        for invite_frnds in event_invite_friends:
                            invite_friends=EventInvitations(type=1,event_id=event_id,user_id=invite_frnds,invite_sent=0,created_at=datetime.datetime.utcnow(),created_by=login_user_id)
                            db.add(invite_friends)
                            db.commit()
    ...
    if token == None or token.strip() == "":
        ...
        if access_token == False:
            ...
            if notification_type == 1: # Nugget
                ...
                # my_followers=db.query(FollowUser).filter(or_(FollowUser.following_userid == login_user_id,FollowUser.follower_userid == login_user_id),FollowUser.status == 1).all()
                # for my_follw in my_followers:
                #     my_frnd_id.append(my_follw.follower_userid)
                #     my_frnd_id.append(my_follw.following_userid)
        ...
        if access_token == False:
            ...
            if not get_notification:
                ...
                if get_notification.user_id == login_user_id:
                    delete_notification=db.query(Notification).filter_by(id = notification_id).update({"status":0,"deleted_datetime":datetime.datetime.utcnow()})
                    db.commit()
        ...
        if checkAuthCode(auth_code,token) == False:
            ...
            if access_token == False:
                ...
                if (user_id in blocked_users) and blocked_users :
                    ...
                    get_follow_user= db.query(FollowUser).filter(FollowUser.following_userid == user_id ,FollowUser.follower_userid == login_user_id).first()
                    ...
                    if not get_user:
                        ...
                        account_verify_status=db.query(VerifyAccounts).filter(VerifyAccounts.user_id == user_id).first()
                        ...
                        result_list={
                                        ...
                                        "name":get_user.display_name if get_user.display_name else "",
                                        "email_id":get_user.email_id if get_user.email_id else "",
                                        "mobile":ProfilePreference(db,login_user_id,get_user.id,'phone_display_status',get_user.mobile_no) if get_user.mobile_no else "",
                                        "dob":ProfilePreference(db,login_user_id,get_user.id,'dob_display_status',get_user.dob) if get_user.dob else "",
                                        "geo_location":ProfilePreference(db,login_user_id,get_user.id,'location_display_status',get_user.geo_location) if get_user.geo_location else "",
                                        "bio_data":ProfilePreference(db,login_user_id,get_user.id,field,get_user.bio_data),
                                        ...
                                        "longitude":get_user.longitude if get_user.longitude else "",
                                        "date_of_join":common_date(get_user.created_at) if get_user.created_at else "",
                                        "user_type":get_user.user_type_master.name if get_user.user_type_master.name else "",
                                        "user_status":get_user.user_status_master.name if get_user.user_status_master.name else "",
                                        "user_status_id":get_user.user_status_id if get_user.user_status_id else "",
                                        ...
                                        "is_friend_request_sender": 1 if get_friend_request and get_friend_request.sender_id == get_user.id else 0,
...
async def listallblockedusers(db:Session=Depends(deps.get_db),token:str=Form(None),search_key:str=Form(None),page_number:str=Form(default=1)):
    if token == None or token.strip() == "":
        ...
        if access_token == False:
            ...
            get_friends=db.query(MyFriends).join(User,MyFriends.receiver_id == User.id,isouter=True).filter(MyFriends.status == 1 ,MyFriends.request_status == 3,MyFriends.sender_id == login_user_id)
           
            if search_key:
                get_friends=get_friends.filter(or_(User.email_id.ilike(search_key),User.display_name.ilike(search_key),User.first_name.ilike(search_key),User.last_name.ilike(search_key)))
    ...
    if token.strip() == "":
        ...
        if access_token == False:
            ...
            if action == 1:  # Block User
                ...
                add_frnd=MyFriends(sender_id=login_user_id,receiver_id=user_id,request_date=datetime.datetime.utcnow(),request_status=3,status_date=datetime.datetime.utcnow(),status=1)
                db.add(add_frnd)
                db.commit()
                if not add_frnd:
                    return {"status":0,"msg":"Failed to update. please try again"}
...
async def getusersettings(db:Session=Depends(deps.get_db),token:str=Form(None)):
    if token == None or token.strip() == "":
        ...
        if access_token == False:
            ...
            user_status_list=s=db.query(UserStatusMaster).filter_by(id = get_user_settings.id).first()
            if user_status_list:
                result_list.update({
            ...
            if user_setting_id !='':
                ...
                if settings:
                    ...
                    if default_melody:
                        ...
                        if file_size > 1000000 and type == 'image' and file_ext != '.gif':
                            compress=1
                            uploaded_file_path=await read_file_upload(file_read,file_ext,compress)
                    ...
                    header_image=settings.meeting_header_image if settings.meeting_header_image else None
                    ...
                    if meeting_header_image:
                        ...
                        if not file_ext in extensions:
                            ...
                            if file_size > 1024:
                                local_file_upload=await read_file_upload(file_read,file_ext,compress=1)
                                header_image=local_file_upload
                            else:
                                local_file_upload=await read_file_upload(file_read,file_ext,compress=None)
                                header_image=local_file_upload
                    ...
                    live_banner=settings.live_event_banner if settings.live_event_banner else None
                    ...
                    talkshow_banner=settings.talkshow_event_banner if settings.talkshow_event_banner else None
                    ...
                    waiting_room=int(waiting_room) if waiting_room != None and waiting_room.isnumeric() else settings.waiting_room
                    online_status=int(online_status) if online_status != None and online_status.isnumeric() else settings.online_status
            ...
            # userlist = json.loads(user_id)
                ...
                for user_id in userlist:
                    ...
                    db.query(MyFriends).filter(or_(
                        and_(MyFriends.sender_id == login_user_id, MyFriends.receiver_id == user_id),
                        and_(MyFriends.sender_id == user_id, MyFriends.receiver_id == login_user_id)
                    )).update({'request_status': 3, 'status_date': datetime.datetime.utcnow(), 'status': 0})
                    
                    # Create new friend request with status blocked
                    model = MyFriends(sender_id=login_user_id, receiver_id=user_id, request_date=datetime.datetime.utcnow(), request_status=3, status_date=datetime.datetime.utcnow(), status=1)
                    db.add(model)
                    db.commit()
                    if model.id:
                        update = True
        ...
        if access_token == False:
            ...
            if get_row_count<1:
                ...
                if event_list:
                    ...
                    for event in event_list:
                        default_melody = db.query(EventMelody).filter_by(id=event.event_melody_id).first()
                        default_av=db.query(EventDefaultAv).filter(EventDefaultAv.event_id == event.id).order_by(EventDefaultAv.id.desc()).first()
                        ...
                        result_list.append({
                                            ...
                                            "start_date":event.start_date_time.strftime('%b %d') if event.start_date_time else "",
                                            "start_time":event.start_date_time.strftime('%I:%M %p') if event.start_date_time else "",
                                            "duration":event.duration,
                                            "no_of_participants": event.no_of_participants,
                                            "banner_image":event.cover_img,
                                            "is_host":1 if login_user_id == event.created_by else 0,
                                            "created_at":event.created_at.strftime('%Y-%m-%d %I:%M %p') if event.created_at else "",
                                            "original_user_name":event.user.display_name if event.created_by else "",
                                            "original_user_id":event.created_by if event.created_by else "",
                                            "original_user_image":event.user.profile_img if event.created_by else "",
                                            "waiting_room":event.waiting_room if event.waiting_room in [0, 1] else waiting_room,
                                            "join_before_host":event.join_before_host if event.join_before_host in [0, 1] else join_before_host,
                                            "sound_notify": event.sound_notify if event.sound_notify in [0, 1] else sound_notify,
                                            "user_screenshare":event.user_screenshare if (event.user_screenshare == 1 or event.user_screenshare == 0)  else user_screenshare,
                                            "event_melody_id":event.event_melody_id,
                                            "melodies":{"path":default_melody.path if default_melody else "",
                                                        "type":default_melody.type if default_melody else "",
                                                        "is_default":None}, # default_melody.event_id is None
                                            "default_host_audio":default_av.default_host_audio if default_av else "",
                                            "default_host_video":default_av.default_host_video if default_av else "",
                                            "default_guest_audio":default_av.default_guest_audio if default_av else "",
                                            "default_guest_video":default_av.default_guest_video if default_av else ""
                                           })
...
async def globalsearchnuggets(db:Session=Depends(deps.get_db),token:str=Form(None),search_key:str=Form(None),page_number:str=Form(1)):
    ...
    if search_key == None:
        ...
        if access_token == False:
            ...
            group_ids=getGroupids(db,login_user_id)
            my_friends=get_friend_requests(db,login_user_id,requested_by=None,request_status=1,response_type=1,search_key=None)
...
async def messageread(db:Session=Depends(deps.get_db),token:str=Form(None),senderid:str=Form(None),receiverid:str=Form(None)):
    ...
    if access_token == False:
        ...
        report = db.query(FriendsChat).filter(FriendsChat.sender_id == senderid,FriendsChat.receiver_id == receiverid,FriendsChat.is_read == 0).update({"is_read":1,"read_datetime":datetime.datetime.utcnow()})
        db.commit()
...
async def getfollowlist(db:Session=Depends(deps.get_db),token:str=Form(None),user_id:str=Form(None),type:str=Form(None,description="1->follower,2->following"),search_key:str=Form(None),location:str=Form(None),gender:str=Form(None),age:str=Form(None),page_number:str=Form(default=1)):
    if token == None or token.strip() == "":
        return {"status":-1,"msg":"Sorry! your login session expired. please login again."}
        ...
        if access_token == False:
            ...
            get_token_details=db.query(ApiTokens).filter(ApiTokens.token == access_token).order_by(ApiTokens.id.desc()).first()
            login_user_id=get_token_details.user_id if get_token_details else None
    ...
    #     return {"status":0,"msg":"Event Banner is missing"}
        ...
        if access_token == False:
            ...
            if not event_title:
                ...
                duration = (datetime.datetime.min + timedelta(seconds=duration)).strftime('%H:%M:%S')
                ...
                new_event = Events(title=event_title,type = 2,ref_id = reference_id,server_id = server_id if server_id else None,
                                   event_type_id = event_type,event_layout_id = 1,duration = duration,created_at = datetime.datetime.utcnow(),
                    ...
                    if event_invite_custom and event_invite_custom != '':
                        for value in event_invite_custom:  # check if e-mail address is well-formed    
                            if check_mail(value) == True:
                                invite_custom = EventInvitations(type=3, event_id=new_event.id, invite_mail=value, invite_sent=0, created_at=datetime.datetime.utcnow(), created_by=login_user_id)
                                db.add(invite_custom)
                                db.commit()
                                to=value
...
async def editliveevent(db:Session=Depends(deps.get_db),token:str=Form(None),event_id:str=Form(None),event_title:str=Form(None),event_type:str=Form(None),event_start_date:Any=Form(None),event_start_time:Any=Form(None),event_banner:UploadFile=File(None),
                              event_invite_mails:str=Form(None,description="Example abc@mail.com,def@mail.com"),event_invite_groups:str=Form(None,description="Example 1,2,3"),event_invite_friends:str=Form(None,description="Example 1,2,3"),
                              delete_invite_mails:str=Form(None,description=" Example abc@mail.com,def@mail.com"),
            ...
            if access_token == False:
                ...
                if event_title.strip() == "":
                    ...
                    new_event=Events(title=event_title,type=3,ref_id=reference_id,server_id = server_id,
                                     event_type_id=event_type,event_layout_id=1,duration=duration,start_date_time=datetime.datetime.utcnow(),
                                     created_at=datetime.datetime.utcnow(),created_by=login_user_id,cover_img=cover_img,status=0)
                    db.add(new_event)
                    db.commit()
                    if new_event:
                        event=get_event_detail(db,new_event.id,login_user_id)
        ...
        if checkAuthCode(auth_code,auth_text) == False:
            ...
            if check_phone :
                ...
                if model:
                    ...
                    update_user=db.query(User).filter_by(id=model.id).update({'user_ref_id': user_ref_id, 'password': hashlib.sha1(user_ref_id.encode()).hexdigest()})
                    db.commit()

                    #Set Default user settings 
                    user_settings_model=UserSettings(user_id=model.id,online_status=1,friend_request='000',nuggets='000',events='000',status=1)
                    db.add(user_settings_model)
                    db.commit()

                    #Set Default user settings
                    friends_group=FriendGroups(group_name="My Fans",group_icon=defaultimage('group_icon'),created_by=model.id,created_at=datetime.datetime.utcnow(),status=1,chat_enabled=0)
                    db.add(friends_group)
                    db.commit()
                    ...
                    if friend_ref_code:
                        ...
                        if len(referrer_ref_id) == 2:
                            ...
                            if referred_user:
                                ...
                                if group:
                                    Followmodel=FollowUser(follower_userid=model.id,following_userid=referred_user.id,created_date=datetime.datetime.utcnow())
                                    db.add(Followmodel)
                                    db.commit()
                    ...
                    if rawcaster_support_id and referred_id != rawcaster_support_id:
                        myfriend=MyFriends(sender_id=rawcaster_support_id,receiver_id=model.id,request_date=datetime.datetime.utcnow(),request_status=1,status_date=None,status=1)
                        db.add(myfriend)
                        db.commit()
...
async def influencerlist(db:Session=Depends(deps.get_db),token:str=Form(None),search_key:str=Form(None),auth_code:str=Form(None,description="SALT+token"),category:str=Form(None,description="influencer category"),page_number:str=Form(default=1)):
    ...
    if checkAuthCode(auth_code,auth_text) ==False:
        ...
        if access_token == False:
            ...
            api_search_key=search_key if search_key and (search_key.strip() != None or search_key.strip()!= "") else None
            ...
            if api_category !=None and api_category !='':
                criteria=criteria.filter(User.influencer_category.ilike("%" + api_category + "%"))
            ...
            if get_row_count < 1:
                ...
                for res in get_result:
                    ...
                    result_list.append({
                        ...
                        "bio_data":ProfilePreference(db,login_user_id,res.id,'bio_display_status',res.bio_data)
                    ...
                    for follow_user_id in follow_userids:
                        ...
                        if users:
                            ...
                            if user and not follow_user and login_user_id != follow_userid:
                                add_follow_user=FollowUser(follower_userid=login_user_id,following_userid=follow_userid,created_date=datetime.datetime.utcnow(),status = 1)
                                db.add(add_follow_user)
                                db.commit()
                                ...
                                if add_follow_user:
                                    if friend_group:
                                        ...
                                        if not friend_group_member:
                                            add_group_member=FriendGroupMembers(group_id=friend_group.id,user_id=login_user_id,added_date=datetime.datetime.utcnow(),added_by=follow_userid,is_admin=0,disable_notification=1,status=1)
