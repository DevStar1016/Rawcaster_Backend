...
class Nuggets(Base):
    ...
    nuggets_id = Column(
        Integer, ForeignKey("nuggets_master.id"), comment=" nuggets_master table ID "
    )
    user_id = Column(
        Integer, ForeignKey("user.id"), comment=" nuggets_master table ID "
    )
    ...
    total_poll_count = Column(Integer,default=0)
    ...
    nugget_status = Column(
        TINYINT, default=1, comment=" 1->Active,2->deleted,3->blocked "
    )
    ...
    nuggets_comments_likes = relationship(
        "NuggetsCommentsLikes", back_populates="nuggets"
    )
