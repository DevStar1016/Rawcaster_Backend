...
class Events(Base):
    ...
    chime_meeting_id=Column(String(50))
    server_id = Column(String(10), default="US1")
    type = Column(
        TINYINT, nullable=False, default=1, comment=" 1 - Event, 2 - Talkshow, 3-Live"
    )
    description = Column(Text)
    event_type_id = Column(
        Integer, ForeignKey("event_types.id"), comment=" event_type table ref id "
    )
    event_layout_id = Column(Integer, ForeignKey("event_layouts.id"))
    event_melody_id = Column(
        Integer, default=1, comment=" id ref from tabelevent_melody "
    )
    ...
    online_payment_details = relationship(
        "OnlinePaymentDetails", back_populates="events"
    )
