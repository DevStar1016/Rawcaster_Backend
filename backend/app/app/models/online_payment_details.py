...
class OnlinePaymentDetails(Base):
    ...
    payment_gateway_request = Column(
        Text, comment=" raw request sent to payment gateway with params "
    )
    payment_gateway_response = Column(
        Text, comment=" raw response received from payment gateway "
    )
    payment_status = Column(
        TINYINT, default=0, comment=" 0->Pending, 1->Success, 2->Failed "
    )
    ...
    payment_gateways = relationship(
        "PaymentGateways", back_populates="online_payment_details"
    )
